...
///
/// The [CustomBoxDecoration] class provides a variety of ways to draw a box.
///
...
/// {@tool snippet}
///
/// The following applies a [CustomBoxDecoration] to a [Container] widget to draw an
...
class CustomBoxDecoration extends Decoration {
  ...
  bool operator ==(Object other) {
    ...
    return other is CustomBoxDecoration &&
        other.color == color &&
        other.image == image &&
        other.border == border &&
...
class _BoxDecorationPainter extends BoxPainter {
  ...
  // void _paintPath(Canvas canvas, Path path, Paint paint, TextDirection? textDirection) {
  //   switch (_decoration.shape) {
  //     case BoxShape.circle:
  //       // assert(_decoration.borderRadius == null);
  //       // final Offset center = rect.center;
  ...
  void _paintShadows(Canvas canvas, Rect rect, TextDirection? textDirection) {
    ...
    //   final Paint paint = boxShadow.toPaint()
    //     ..maskFilter = MaskFilter.blur(BlurStyle.outer, boxShadow.blurSigma);
    //   // final Rect bounds = rect.shift(boxShadow.offset).inflate(boxShadow.spreadRadius);
    //
    //   var path = Path();
    //   path
    //     ..fillType = PathFillType.evenOdd
    //     ..addRect(rect.shift(boxShadow.offset).inflate(boxShadow.spreadRadius));
    //
    //   if (_decoration.borderRadius == null) {
    //     path.addRect(rect);
    //   } else {
    ...
    //   }
    //
    //   final Rect bounds = rect.shift(boxShadow.offset);
    //   _paintPath(canvas, path, paint, textDirection);
    // }

    for (final BoxShadow boxShadow in _decoration.boxShadow!) {
      // final Paint paint = boxShadow.toPaint();
      final Paint paint = boxShadow.toPaint()..maskFilter = MaskFilter.blur(BlurStyle.outer, boxShadow.blurSigma);
      // final Rect bounds = rect.shift(boxShadow.offset).inflate(boxShadow.spreadRadius);
      final Rect bounds = rect.shift(boxShadow.offset);
