...
ZenithXdgSurface::ZenithXdgSurface(wlr_xdg_surface* xdg_surface) : xdg_surface{xdg_surface} {
	...
	switch (xdg_surface->role) {
		case WLR_XDG_SURFACE_ROLE_NONE:
			...
			break;
		case WLR_XDG_SURFACE_ROLE_TOPLEVEL: {
			toplevel = ZenithXdgToplevel(xdg_surface->toplevel);
			auto* server = ZenithServer::instance();
			server->toplevels.insert(std::make_pair(zenith_surface()->id, &toplevel));
			break;
		}
		case WLR_XDG_SURFACE_ROLE_POPUP:
			popup = ZenithXdgPopup(xdg_surface->popup);
			break;
	}
}
...
void surface_commit(wl_listener* listener, void* data) {
	...
	int texture_id;
	if (wlr_surface_has_buffer(zenith_surface->surface)) {
...
void xdg_surface_map2(wl_listener* listener, void* data) {
	...
	wlr_gles2_texture_attribs attribs{};
	wlr_gles2_texture_get_attribs(surface->surface->buffer->texture, &attribs);

	FlutterEngineRegisterExternalTexture(ZenithServer::instance()->embedder_state->engine,
	                                     (int64_t) attribs.tex);
...
void xdg_surface_destroy2(wl_listener* listener, void* data) {
	...
	zenith_surface->surface->data = nullptr;
	...
	if (keyboard != nullptr) {
		...
		for (auto& text_input: server->text_inputs) {
			if (wl_resource_get_client(text_input->wlr_text_input->resource) == client &&
				...
				text_input->enter(base_xdg_surface);
			}
		}
	}
}
